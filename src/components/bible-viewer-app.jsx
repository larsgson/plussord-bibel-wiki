...
import { plussOrdObj } from '../constants/readingPlan'
...
const BibleviewerApp = () => {
  ...
  const curSerie = plussOrdObj
  const nbrEpisodes = plussOrdObj.episodeList.length
  const firstDateOfPlan = Date.parse(plussOrdObj.beginDate)
  ...
  useEffect(() => {
    apiGetStorage(`${plussOrdObj.uniqueID}.completed`).then((value) => {
  ...
  const localSetCompletedList = (inx) => {
    setCompletedList((prev) => {
      ...
      apiSetStorage(`${plussOrdObj.uniqueID}.completed`,newList)
      return newList
    })
  }
